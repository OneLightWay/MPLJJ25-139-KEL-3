...
class _CuacaPageState extends State<CuacaPage> {
  final String _apiKey = '3d403ffd4da1a967b5471f0fcde29367'; // API key OpenWeatherMap Anda
  final String _currentWeatherApiBaseUrl = 'https://api.openweathermap.org/data/2.5/weather';
  final String _forecastApiBaseUrl = 'https://api.openweathermap.org/data/2.5/forecast';
  final String _geocodingApiBaseUrl = 'http://api.openweathermap.org/geo/1.0/direct';
  ...
  String _cleanLocationName(String name) {
    name = name.replaceAll(RegExp(r'^(KOTA|KABUPATEN)\s+', caseSensitive: false), '').trim();
    return name;
  }
    ...
    try {
      ...
      if (snapshot.exists && snapshot.value is Map) {
        ...
        if (kabupatenRaw != null && provinsi != null) {
          ...
          print("Data kabupaten atau provinsi tidak ditemukan untuk pengguna ini.");
      ...
      if (firebaseLat != null && firebaseLon != null) {
        ...
        print('Koordinat diambil dari Firebase: Lat=$_latitude, Lon=$_longitude');
        ...
        final geocodingUrl = Uri.parse('$_geocodingApiBaseUrl?q=$locationQuery&limit=1&appid=$_apiKey');
        ...
        if (geocodingResponse.statusCode == 200) {
          ...
          if (geoData.isNotEmpty) {
            ...
            print('Koordinat ditemukan via geocoding: Lat=$_latitude, Lon=$_longitude');
            if (geoData[0]['name'] != null && geoData[0]['country'] != null) {
               _userLocationDisplay = "${geoData[0]['name']}, ${geoData[0]['country']}";
            }
          } else {
            throw Exception("Koordinat tidak ditemukan untuk lokasi '$locationQuery' via geocoding.");
          }
        } else {
          throw Exception('Gagal mendapatkan koordinat via geocoding: ${geocodingResponse.statusCode} - ${geocodingResponse.body}');
        }
      } else {
        throw Exception("Nama lokasi (kabupaten) tidak tersedia untuk geocoding atau di Firebase.");
      }
      ...
      final currentWeatherUrl = Uri.parse('$_currentWeatherApiBaseUrl?lat=$_latitude&lon=$_longitude&appid=$_apiKey&units=metric&lang=id');
      ...
      if (currentWeatherResponse.statusCode == 200) {
        ...
        if (_currentWeather!['name'] != null && _currentWeather!['sys']['country'] != null) {
          _userLocationDisplay = "${_currentWeather!['name']}, ${_currentWeather!['sys']['country']}";
        }
      } else {
        throw Exception('Gagal memuat cuaca saat ini: ${currentWeatherResponse.statusCode} - ${currentWeatherResponse.body}');
      }

      final forecastUrl = Uri.parse('$_forecastApiBaseUrl?lat=$_latitude&lon=$_longitude&appid=$_apiKey&units=metric&lang=id');
      final forecastResponse = await http.get(forecastUrl);

      if (forecastResponse.statusCode == 200) {
        _forecastWeatherList = json.decode(forecastResponse.body)['list'];
      } else {
        throw Exception('Gagal memuat perkiraan cuaca: ${forecastResponse.statusCode} - ${forecastResponse.body}');
      }
    } catch (e) {
      _errorMessage = 'Gagal memuat data cuaca: $e. Pastikan koneksi internet Anda stabil, kunci API Anda benar, dan koordinat lokasi tersedia.';
  ...
  String _getWeatherIcon(dynamic weatherData) {
    ...
    } else if (description.contains('clouds') || description.contains('berawan')) {
      if (description.contains('scattered clouds') || description.contains('few clouds')) {
        return '${iconPath}cerah-berawan.png';
      }
      return '${iconPath}berawan.png';
    } else if (description.contains('rain') || description.contains('drizzle') || description.contains('hujan')) {
      return '${iconPath}hujan.png';
    } else if (description.contains('thunderstorm') || description.contains('badai')) {
      return '${iconPath}hujan-badai.png';
    } else if (description.contains('snow') || description.contains('salju')) {
      return '${iconPath}salju.png';
    } else if (description.contains('mist') || description.contains('fog') || description.contains('kabut')) {
  ...
  List<Map<String, dynamic>> _getDailyForecasts(List<dynamic>? forecastList) {
    ...
    for (var item in forecastList) {
      DateTime forecastTime = DateTime.fromMillisecondsSinceEpoch(item['dt'] * 1000);
      String dateKey = DateFormat('yyyy-MM-dd').format(forecastTime);

      if (forecastTime.isAfter(now.add(const Duration(hours: 3))) && !dailyForecasts.containsKey(dateKey)) {
    ...
    sortedForecasts.sort((a, b) => (a['date'] as DateTime).compareTo(b['date'] as DateTime));
  ...
  Widget build(BuildContext context) {
    List<Map<String, dynamic>> dailyForecasts = _getDailyForecasts(_forecastWeatherList);
                        ...
                        Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 12),
                              Text(
                                _currentWeather != null && _currentWeather!['weather'] != null
                              ...
                              Text(
                                _currentWeather != null
                                    ? DateFormat('EEEE, dd MMMMWriteHeader', 'id_ID')
                              ...
                              const SizedBox(height: 12),
                              Text(
                                _currentWeather != null ? "${_currentWeather!['main']['temp']?.round()}℃" : "--℃",
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          DateFormat('EEEE, dd MMMMWriteHeader', 'id_ID').format(item['date']),
                                          style: const TextStyle(color: Colors.grey),
                                        ),
                                      ],
                                    ),
