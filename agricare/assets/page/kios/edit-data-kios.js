...
function loadEdit() {
    HELPER.ajax({
        url: BASE_URL + 'AkunKios/getKios',
        data: { user_id: HELPER.getItem('user_id') },
        success: function (res) {

            var dataKios = res.data;
            
            loadMap(dataKios)
            if (!HELPER.isNull(dataKios.kios_banner)) {
                $('.user-foto').attr('src', BASE_ASSETS + 'kiosBanner/thumbs/' + dataKios.kios_banner);
            }

            loadProvinsi(dataKios.kios_province_id)
            if (dataKios.kios_regency_id != null && dataKios.kios_district_id != null && dataKios.kios_village_id != null) {
                loadKota(dataKios.kios_province_id, dataKios.kios_regency_id)
                loadKecamatan(dataKios.kios_regency_id, dataKios.kios_district_id)
                loadKelurahan(dataKios.kios_district_id, dataKios.kios_village_id)
            }

            $('#input_user_is_kios').val(3)
            $('#input_user_id').val(HELPER.getItem('user_id'))
            $('#input_name').val(dataKios.kios_nama)
            $('#input_email').val(HELPER.getItem('user_email'))
            $('#input_telepon').val(HELPER.getItem('user_telepon'))
            $('#input_alamat').val(dataKios.kios_alamat)
            if (dataKios.kios_lat && dataKios.kios_long) {
                $('#input_user_lat').val(dataKios.kios_lat)
                $('#input_user_long').val(dataKios.kios_long)
            }
...
function save(name) {
    if ($('#input_name').val() != "" && $('#input_telepon').val() != "" && $('#input_alamat').val() != "" && $('#input_user_lat').val() != "" && $('#input_user_long').val() != "") {
        ...
        HELPER.save({
            url: BASE_URL + 'AkunKios/editKios',
            cache: false,
            data: formData,
            contentType: false,
            processData: false,
...
function loadMap(dataKios) {
    ...
    var now_latlng = null;
    if (HELPER.isNull(dataKios.kios_lat) == false || HELPER.isNull(dataKios.kios_long) == false) {
        now_latlng = new L.LatLng(dataKios.kios_lat, dataKios.kios_long);
    } else {
