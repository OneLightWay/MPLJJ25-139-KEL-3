...
function loadPage() {
    ...
    HELPER.initLoadMore({
        perPage: 10,
        urlExist: BASE_URL + 'CustomerFeedback/listFeedbackSalesExist',
        dataExist: {
            user_id : sales_id,
        ...
        urlMore: BASE_URL + 'CustomerFeedback/listFeedbackSales',
        dataMore: {
            user_id: sales_id,
            status : filter_status,
            sort : filter_sort,
            ...
            myQueue.enqueue(function (next) {
                ...
                $.each(customer_feedback.data, function (i, v) {
                    ...
                    if (moment(v.customer_feedback_created_at).isSame(moment(), 'day')) {
                        timeDisplay = 'Today, ' + moment(v.customer_feedback_created_at).format('HH:mm');
                    }
                    else {
                        timeDisplay = moment(v.customer_feedback_created_at).format('ddd, DD MMM YYYY');
                    }
                    ...
                    $('.list_customer_feedback').append(`
                        <div class="content bottom-20"> 
                            <a href="javascript:void(0)" class="caption round-medium shadow-large bg-theme bottom-15 show-overlay" onclick="feedbackDetail('${v.customer_feedback_id}')">
                                <div class="content" style="display: flex; margin-top: 15px; margin-bottom: 10px">
                                    <div style="width: 75%; display : flex; margin-block: auto">
                                        <img src="${BASE_ASSETS + 'user_mobile/thumbs/' + v.user_foto}" onerror="this.src='./assets/images/noimage.png'" style="margin-right: 15px ;height: 50px; height : 50px; aspect-ratio : 1/1; border-radius : 25px;"/>
                                        <div style="width : 50%; display: flex; flex-direction: column">
                                            <h2 class="font-14 color-custom-dark" style="margin-bottom: 0">${v.user_nama}</h2>
                                            <span style="color:#707070">${timeDisplay}</span>
                                        </div>
                                    </div>
                                    <div class="color-custom-dark" style="width : 25%; text-align : right; margin-block: auto;">
                                        ${statusLabel}
                                    </div>
                                </div>
                                <div class="content left-20" style="margin-bottom: 10px">
                                    <div class="">
                                        <p class="color-custom-dark" style="margin-bottom : 0 !important">` + getSafe(() => data.deskripsi, '') + `</p>
                                    </div>
                                </div>
                                <div class="content-footer">
                                    <span class="bhsConf-ketuk_detail">Lihat selengkapnya</span>
