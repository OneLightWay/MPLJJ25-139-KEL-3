...
function showDetailInfo() {
    ...
    var artikel_perawatan = atob(HELPER.getItem('detail_informasi_perawatan'));
    var artikel_pencegahan = atob(HELPER.getItem('detail_informasi_pencegahan'));
    var artikel_ciri = atob(HELPER.getItem('detail_informasi_ciri'));
    ...
    $('.detail-info-artikel_content').html(HELPER.nullConverter(artikel_content))
    $('.detail-info-artikel_perawatan').html(HELPER.nullConverter(artikel_perawatan))
    $('.detail-info-artikel_pencegahan').html(HELPER.nullConverter(artikel_pencegahan))
    $('.detail-info-artikel_ciri').html(HELPER.nullConverter(artikel_ciri))
    preventHrefWysiwyg(['detail-info-artikel_content', 'detail-info-artikel_perawatan', 'detail-info-artikel_pencegahan', 'detail-info-artikel_ciri'])
...
function hamaArtikel() {
    var artikel_id = HELPER.getItem('artikel_id');
    var artikel_hama_kategori = HELPER.getItem('artikel_hama_kategori');
    // onPage('hpt-artikel')
    HELPER.block()
    HELPER.ajax({
        url: BASE_URL + 'Artikel/getHamaArtikel',
        data: {
            kategori: artikel_id,
        },
        ...
        complete: function (res) {
            if (res.total > 0) {
                ...
                $.each(res.data, function(i, v) {
                    ...
                    //             <img src="`+img+`" onerror="this.src='./assets/images/noimage.png'" data-src="" class="top-0 symbol w-100px example-image border border-secondary detail-artikel_image_name rounded" alt="img" style="border-radius:5px; height:94px;">
                    //         </a>`;
                    // galeri+=`<a class="default-link" data-lightbox="`+img+`" href="`+img+`" title="Vynil and Typerwritter">
                    //             <img src="`+img+`" onerror="this.src='./assets/images/noimage.png'" data-src="" class="preload-image responsive-image" alt="img">
                    //         </a>`;
