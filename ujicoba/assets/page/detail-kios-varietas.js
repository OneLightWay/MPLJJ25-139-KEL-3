$(function () {
    ...
    setTimeout(function () {
        ...
        if (HELPER.getItem('from_page') || HELPER.getItem('from_page_detail_k_v')) {
            fromPage = HELPER.getItem('from_page_detail_k_v') ? HELPER.getItem('from_page_detail_k_v') : HELPER.getItem('from_page');
            setTimeout(function () {
                $('.back-button').on('click', function () {
                    setTimeout(function () {
                        if (fromPage != 'detail-rekomendasi-produk') {
                            onPage(fromPage)
                        }else{
...
function loadKios(id) {
    ...
    HELPER.ajax({
        url: BASE_URL + 'Kios/readKiosVarietas',
        data: {
            user_id: HELPER.getItem('kios_varietas_user_id'),
            user_lat: jarak_lat,
        ...
        complete: function (res) {
            ...
            setTimeout(function () {
                ...
                $('.detail-kios-alamat').text(HELPER.nullConverter(res.kios_alamat))
...
function detailVarietasKios(kios_id) {
    HELPER.setItem('detail_varietas_kios', kios_id)
    HELPER.setItem('from_page_d_kios_v', 'detail-kios-varietas')
    onPage('detail-produk-varietas-kios')
}

function showProduct() {
    var user_id = HELPER.getItem('kios_varietas_user_id');
    var varietas_id = HELPER.getItem('benih_varietas');

    HELPER.block()
    HELPER.ajax({
        url: BASE_URL + 'Kios/listBenih',
        data: {
            user: user_id,
            ...
            if (res.total > 0) {
                ...
                $.each(res.data, function (i, v) {
                    ...
                    slider += `<div class="item bg-theme round-small">
                                <a href="javascript:;" class="color-theme caption m-0" style="height:200px; width:220px;" onclick="detailVarietasKios('`+ v.kios_varietas_id + `')">
                                    ...
                                    <div class="bottom-5">
                                        <h5 class="bottom-0 lh-20 center-text">`+ v.kios_varietas_nama + `</h5>
                                        <h5 class="bottom-10 lh-20 font-14 center-text">`+ v.kios_varietas_kemasan + ` Kg</h5>
                                    </div>    
...
function showLink(kios_link_id) {
    /* HELPER.ajax({
        ...
        data: {
            // user: user_id,
            id: HELPER.getItem('kios_varietas_user_id'),
        },
        complete: function (res) {
            console.log(res)
            if (res.total > 0) {
