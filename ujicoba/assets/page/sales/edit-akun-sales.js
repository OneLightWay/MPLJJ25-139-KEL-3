...
$(function () {
    if (checkIsPetugas()) {
        ...
        $(".menu-qcs").show();
        $("#footer-menu")
            .removeClass("footer-menu-3-icons")
            .addClass("footer-menu-2-icons");
    }
    loadEdit();
});
...
function loadEdit() {
    ...
    if (
        HELPER.getItem("user_regency_id") != null &&
        HELPER.getItem("user_district_id") != null &&
        HELPER.getItem("user_village_id") != null
    ) {
        loadKota(
            HELPER.getItem("user_province_id"),
            HELPER.getItem("user_regency_id")
        );
        loadKecamatan(
            HELPER.getItem("user_regency_id"),
            HELPER.getItem("user_district_id")
        );
        loadKelurahan(
            HELPER.getItem("user_district_id"),
            HELPER.getItem("user_village_id")
        );
    }

    $("#input_user_is_petugas").val(HELPER.getItem("is_petugas"));
...
function loadMap() {
    ...
    var now_latlng = null;
    if (
        HELPER.isNull(HELPER.getItem("user_lat")) == false ||
        HELPER.isNull(HELPER.getItem("user_long")) == false
    ) {
        now_latlng = new L.LatLng(
            HELPER.getItem("user_lat"),
            HELPER.getItem("user_long")
        );
    } else {
