	...
	if ($('#input_luas_lahan').val() > "0.00" && $('#input_varietas').val() != "" && $('#input_jarak_antar_tanam').val() != "" && $('#input_jarak_dalam_barisan').val() != "0.00" && $('#input_jumlah_benih').val() > "0.00") {
		...
		HELPER.ajax({
			...
			complete: function (res) {
				try {
					FirebasePlugin.logEvent("cal_benih_"+$('#input_varietas_nama').val(), {nk_user_id: HELPER.getItem('user_id'), nk_luas: $('#input_luas_lahan').val(), nk_jenis: $('#input_varietas').val()});
					facebookConnectPlugin.logEvent("cal_benih", {nk_user_id: HELPER.getItem('user_id'), nk_luas: $('#input_luas_lahan').val(), nk_jenis: $('#input_varietas').val()}, 1, ()=>{}, ()=>{})
				} catch (error) {
				}
				...
				setTimeout(function () {
					onPage('detail-kebutuhan-benih');
				}, 300)
				$('page-detail').hide()
			}
		})
	} else {
		FirebasePlugin.logEvent("cal_benih_failed", {nk_user_id: HELPER.getItem('user_id'), nk_user_province: HELPER.getItem('user_province_name'), nk_user_region: HELPER.getItem('user_region_name')});
		Swal.fire(
			'Pemberitahuan',
